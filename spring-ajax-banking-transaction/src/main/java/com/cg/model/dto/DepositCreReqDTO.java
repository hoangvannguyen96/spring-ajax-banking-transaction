        ...
        if (transactionAmountStr.length() == 0) {
            ...
            if (!transactionAmountStr.matches("\\d+")) {
                ...
                if (transactionAmount.compareTo(BigDecimal.valueOf(100000L)) <= 0) {
                    errors.rejectValue("transactionAmount", "transactionAmount.min", "Số tiền muốn nạp thấp nhất là 100.000 VND");
                }
                else {
                    if (transactionAmount.compareTo(BigDecimal.valueOf(100000000L)) > 0) {
                        errors.rejectValue("transactionAmount", "transactionAmount.max", "Số tiền muốn nạp tối đa là 100.000.000 VND");
                    }
                }
            }
        }
